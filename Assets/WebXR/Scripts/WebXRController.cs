    ...
    public class WebXRController : MonoBehaviour
    {
        ...
        public float GetAxis(string action)
        {
            for (var i = 0; i < inputMap.inputs.Count; i++)
            {
                ...
                {
                    if (WebXRManager.unityXrPresent && !input.unityInputIsButton)
                    {
        ...
        {
            if (WebXRManager.unityXrPresent)
            {
        ...
        void Update()
        {
            ...
            if (!WebXRManager.unityXrPresent) return;
            ...
            if (this.simulate3dof)
            {
                ...
                hmdDevice.TryGetFeatureValue(CommonUsages.devicePosition, out var hmdPos);
                hmdDevice.TryGetFeatureValue(CommonUsages.deviceRotation, out var hmdRot);
                handDevice.TryGetFeatureValue(CommonUsages.deviceRotation, out var handRot);

                _t.localPosition = applyArmModel(
                    hmdPos, // we use head position as origin
                    handRot,
                    hmdRot);
                _t.localRotation = handRot;
            }
            else
            {
                var handDevice = InputDevices.GetDeviceAtXRNode(handNode);
                handDevice.TryGetFeatureValue(CommonUsages.devicePosition, out var handPos);
                handDevice.TryGetFeatureValue(CommonUsages.deviceRotation, out var handRot);

                _t.localPosition = handPos;
                _t.localRotation = handRot;
            }
