...
namespace WebXR
{
    ...
    public class WebXRManager : MonoBehaviour
    {
        ...
        public static bool unityXrPresent
        {
            get
            {
        ...
        private void SetTrackingSpaceType()
        {
            if (unityXrPresent)
            {
                var value = TrackingOriginModeFlags.Unknown;
                switch (TrackingSpace)
                {
                    case TrackingSpaceType.RoomScale:
                        value = TrackingOriginModeFlags.Floor;
                        break;
                    case TrackingSpaceType.Stationary:
                        value = TrackingOriginModeFlags.Device;
                        break;
                }

                var activeSubsys = new List<XRInputSubsystem>();
                ...
                foreach (var subsys in activeSubsys)
                {
                    ...
                    if (!subsys.TrySetTrackingOriginMode(value))
                    {
                        ...
                        //Debug.Log("Tracking Space: (" + (subsys.subsystemDescriptor == null ? "?" : subsys.subsystemDescriptor.id) + ") " + subsys.GetTrackingOriginMode());
                        Debug.Log("Tracking Space: " + subsys.GetTrackingOriginMode());
